...
class YOLODetector:
    def __init__(self, model_path='best_int8.tflite', conf_threshold=0.25):
        ...
        # Valeurs letterbox initialisées
        self._scale = 1.0
        self._pad_x = 0
        self._pad_y = 0
        self._orig_w = self.input_size
        self._orig_h = self.input_size
    ...
    def preprocess(self, frame):
        """Letterbox : redimensionne sans déformer, ajoute des bandes noires"""
        h, w = frame.shape[:2]
        scale = min(self.input_size / w, self.input_size / h)
        new_w, new_h = int(w * scale), int(h * scale)

        resized = cv2.resize(frame, (new_w, new_h))
        img = np.full((self.input_size, self.input_size, 3), 114, dtype=np.uint8)

        pad_x = (self.input_size - new_w) // 2
        pad_y = (self.input_size - new_h) // 2
        img[pad_y:pad_y+new_h, pad_x:pad_x+new_w] = resized

        # Sauvegarder pour reconvertir les coordonnées
        self._scale = scale
        self._pad_x = pad_x
        self._pad_y = pad_y
        self._orig_w = w
    ...
    def postprocess(self, output, orig_w, orig_h):
        ...
        for box, conf, cls_id in zip(boxes, confidences, class_ids):
            cx, cy, w, h = box
            # Coordonnées dans l'espace letterbox → pixels originaux
            x1 = int(((cx - w/2) * self.input_size - self._pad_x) / self._scale)
            y1 = int(((cy - h/2) * self.input_size - self._pad_y) / self._scale)
            x2 = int(((cx + w/2) * self.input_size - self._pad_x) / self._scale)
            y2 = int(((cy + h/2) * self.input_size - self._pad_y) / self._scale)
